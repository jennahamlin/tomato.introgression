...
#adjustedp<-p.adjust(mydata$Pvalue, "bonferroni", n = length(mydata$Pvalue))
...
#subset and just plot the values which are significant for first visulization
#mydata<-subset(mydata, adjustedp<0.06, select=c(group, Dstat.alwaysABBAminusBABA))
...
mydata<-read.csv('matingsystem.trios.withLA0400.nopen.csv', header = T)
#only keep rows in which Dsites is greater than 20
mydata<- mydata[which(mydata$Dsites > 20),]
#calculate the adjusted pvalue via bonferroniicorrection
...
ggplot()+
  geom_point(data=mydata, color= ifelse(mydata$Pvalue<0.06, 'deepskyblue', 'lightgray'), 
             aes(x=group,y=Dstat.alwaysABBAminusBABA ), position = position_jitter(w = 0.35, h = 0.35))+
  #geom_quasirandom(data=mydata, color= ifelse(adjustedp<0.05, 'deepskyblue', 'lightgray'), 
  #              aes(x=group,y=Dstat.alwaysABBAminusBABA ), show.legend =F, cex=1,
  #             position = position_jitter(w = 0.3, h = 0.3))+
  labs(y="Genome-wide D-statistic (mean with 95% CI)")+
...
arcSI.arcSC.pimSC<-slice(rawdat,1:5234) #1
habSI.habSC.pimSC<-slice(rawdat,5235:11034 )#2 
perSI.perSC.pimSC<-slice(rawdat,16022:20204)#4

#all data
arcSI.arcSC.pimSC<-slice(rawdat,1:8027) #1
habSI.habSC.pimSC<-slice(rawdat,8028:16054 )#2 
perSI.perSC.pimSC<-slice(rawdat,24082:32108)#4 
